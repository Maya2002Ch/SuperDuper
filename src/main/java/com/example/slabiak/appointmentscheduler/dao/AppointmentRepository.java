...
public interface AppointmentRepository extends JpaRepository<Appointment, Integer> {
    ...
    @Query("select a from Appointment a where a.teacher.id = :providerId")
    ...
    @Query("select a from Appointment a where  a.status='SCHEDULED' and (a.customer.id = :userId or a.teacher.id = :userId)")
    List<Appointment> findScheduledByUserId(@Param("userId") int userId);

    @Query("select a from Appointment a where a.teacher.id = :providerId and  a.start >=:dayStart and  a.start <=:dayEnd")
    ...
    @Query("select a from Appointment a where a.status = 'SCHEDULED' and :date >= a.end and (a.customer.id = :userId or a.teacher.id = :userId)")
    ...
    @Query("select a from Appointment a where a.status = 'FINISHED' and :date >= a.end and (a.customer.id = :userId or a.teacher.id = :userId)")
    ...
    @Query("select a from Appointment a inner join a.work w where a.status = 'SCHEDULED' and a.customer.id <> :customerId and a.teacher.id= :providerId and a.start >= :start and w.id = :workId")
